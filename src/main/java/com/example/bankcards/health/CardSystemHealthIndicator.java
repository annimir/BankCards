...
/**
 * Кастомный индикатор состояния системы для {@code /actuator/health}.
 *
 * <p>Дополняет стандартный ответ {@code UP/DOWN} доменными метриками:
 * общее число карт, активные, истёкшие и количество устаревших refresh токенов.
 * Ops-команда видит реальное состояние домена без прямого доступа к БД.
 *
 * <p>При недоступности БД возвращает {@code DOWN} с деталью исключения.
 */
@Slf4j
